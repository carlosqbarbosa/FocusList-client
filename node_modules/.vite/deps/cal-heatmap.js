...
function extent(values2, valueof) {
  ...
  if (valueof === void 0) {
    for (const value of values2) {
      if (value != null) {
        if (min5 === void 0) {
          if (value >= value) min5 = max6 = value;
        } else {
...
function quickselect(array4, k2, left = 0, right = Infinity, compare) {
  ...
  while (right > left) {
    ...
    let j = right;
    swap(array4, left, k2);
    if (compare(array4[right], t) > 0) swap(array4, left, right);
    while (i < j) {
      swap(array4, i, j), ++i, --j;
      while (compare(array4[i], t) < 0) ++i;
      while (compare(array4[j], t) > 0) --j;
    }
    if (compare(array4[left], t) === 0) swap(array4, left, j);
    else ++j, swap(array4, j, right);
    if (j <= k2) left = j + 1;
    if (k2 <= j) right = j - 1;
  }
  return array4;
...
treeProto.copy = function() {
  ...
  if (!node2) return copy3;
  if (!node2.length) return copy3._root = leaf_copy(node2), copy3;
...
var validateArgumentsLength$1 = function(passed, required2) {
  if (passed < required2) throw new $TypeError$7("Not enough arguments");
  return passed;
};
  ...
  call$d(task, global$7, function() {
    ...
    if (IS_UNHANDLED) {
      result = perform$3(function() {
        ...
        } else dispatchEvent2(UNHANDLED_REJECTION, promise2, value);
      });
...
var onHandleUnhandled = function(state) {
  call$d(task, global$7, function() {
    ...
    } else dispatchEvent2(REJECTION_HANDLED, promise2, state.value);
  });
};
...
var promiseResolve$1 = function(C3, x3) {
  anObject$7(C3);
  if (isObject$c(x3) && x3.constructor === C3) return x3;
...
objectDefineProperties.f = DESCRIPTORS$5 && !V8_PROTOTYPE_DEFINE_BUG ? Object.defineProperties : function defineProperties(O, Properties) {
  ...
  var index2 = 0;
  var key;
  while (length2 > index2) definePropertyModule$1.f(O, key = keys4[index2++], props[key]);
  return O;
};
...
defineIterator$1(String, "String", function(iterated) {
  ...
  var index2 = state.index;
  var point6;
  if (index2 >= string.length) return createIterResultObject$1(void 0, true);
...
$$g({ target: "Array", proto: true, forced: !HAS_SPECIES_SUPPORT$2 }, {
  slice: function slice5(start2, end) {
    ...
    for (n = 0; k2 < fin5; k2++, n++) if (k2 in O) createProperty$3(result, n, O[k2]);
    result.length = n;
    return result;
  }
});
...
var arraySpeciesConstructor$1 = function(originalArray) {
  ...
  if (isArray$1(originalArray)) {
    ...
    if (isConstructor$1(C3) && (C3 === $Array$2 || isArray$1(C3.prototype))) C3 = void 0;
...
var arraySliceSimple = function(O, start2, end) {
  ...
  var n = 0;
  for (; k2 < fin5; k2++, n++) createProperty$2(result, n, O[k2]);
  result.length = n;
  return result;
};
  ...
  concat: function concat(arg) {
    ...
    for (i = -1, length2 = arguments.length; i < length2; i++) {
      ...
      if (isConcatSpreadable(E2)) {
        ...
        for (k2 = 0; k2 < len; k2++, n++) if (k2 in E2) createProperty$1(A5, n, E2[k2]);
      } else {
...
var getSortCompare = function(comparefn) {
  return function(x3, y3) {
    ...
    if (comparefn !== void 0) return +comparefn(x3, y3) || 0;
...
$$9({ target: "Array", proto: true, forced: FORCED }, {
  sort: function sort2(comparefn) {
    ...
    if (STABLE_SORT) return comparefn === void 0 ? nativeSort(array4) : nativeSort(array4, comparefn);
    var items = [];
    var arrayLength = lengthOfArrayLike$1(array4);
    var itemsLength, index2;
    for (index2 = 0; index2 < arrayLength; index2++) {
      if (index2 in array4) push$2(items, array4[index2]);
    }
    internalSort(items, getSortCompare(comparefn));
    itemsLength = lengthOfArrayLike$1(items);
    index2 = 0;
    while (index2 < itemsLength) array4[index2] = items[index2++];
    while (index2 < arrayLength) deletePropertyOrThrow(array4, index2++);
    return array4;
  }
});
...
function createScalePow(key, channels, { exponent: exponent2 = 1, ...options }) {
  return createScaleQ(key, pow2().exponent(exponent2), channels, { ...options, type: "pow" });
}
...
function createScaleQuantize(key, channels, {
  ...
  if (range3 === void 0) {
    ...
    if (thresholds[thresholds.length - 1] >= max6) thresholds.pop();
...
function inferLogDomain(channels) {
  for (const { value } of channels) {
    if (value !== void 0) {
      for (let v2 of value) {
        if (v2 > 0) return inferDomain$1(channels, positive);
        if (v2 < 0) return inferDomain$1(channels, negative);
      }
    }
  }
  return [1, 10];
...
function createScaleD(key, scale3, transform2, channels, {
  ...
  if (descending(min5, max6) < 0) [min5, max6] = [max6, min5], reverse2 = !reverse2;
  ...
  if (symmetric) {
    ...
    if (mindelta < maxdelta) min5 = transform2.invert(mid - maxdelta);
    else if (mindelta > maxdelta) max6 = transform2.invert(mid + mindelta);
  }
...
function maybeRound(scale3, channels, options, key) {
  let { round } = options;
  if (round !== void 0) scale3.round(round = !!round);
...
function inferScaleType(key, channels, { type: type2, domain, range: range3, scheme: scheme28, pivot, projection: projection2 }) {
  ...
  if ((key === "x" || key === "y") && projection2 != null) type2 = typeProjection;
...
function exposeScale({ scale: scale3, type: type2, domain, range: range3, interpolate, interval: interval2, transform: transform2, percent, pivot }) {
  ...
  return {
    ...
    domain: slice6(domain),
    // defensive copy
    ...range3 !== void 0 && { range: slice6(range3) },
    // defensive copy
  ...
  !function(t, e) {
    ...
    var _ = function() {
      ...
      return m2.parse = function(t4) {
        ...
        if ($3 === c4) return this.set(c4, this.$M + r2);
  ...
  !function(t, i) {
    ...
    return function(s2, f, n) {
      ...
      u4.utcOffset = function(s3, f2) {
        ...
        if (f2) return o2.$offset = u5, o2.$u = 0 === s3, o2;
        if (0 !== s3) {
